...
public class TreeController {
    ...
    public TreeController() {
        ...
        for (int i = 0; i < GRID_HEIGHT; i++) {
            for (int j = 0; j < GRID_WIDTH; j++) {
                trees[i][j] = new Tree(
                    TreeImageFactory.getTreeImage(TreeType.WINTER));
            }
        }
    }
    ...
    public void makeTreeLeafy(int row, int col) {
        if (!trees[row][col].isLeafy()) {
            trees[row][col].makeLeafy(
                TreeImageFactory.getTreeImage(TreeType.SUMMER));
            leafyTreeCount++;
        }
    }

    public javafx.scene.image.Image getTreeImage(int row, int col) {
